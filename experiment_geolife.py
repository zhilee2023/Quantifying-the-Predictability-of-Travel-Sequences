...
from beijing_boundary import DEFAULT_BEIJING_GEOJSON_URL
...
from geolife_preprocess import (
    ...
    discover_labeled_users,
    extract_segment_arrays,
    grid_discretize,
    list_beijing_user_ids,
    markov_metrics,
    parse_geolife_labels,
    symbolic_ctw_metrics,
)

WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_GEOLIFE_DATA_DIR = WORKSPACE_ROOT / "Geolife Trajectories 1.3" / "Data"
DEFAULT_SF_DATA_PATH = WORKSPACE_ROOT / "sf_dataset.csv"
DEFAULT_GEOLIFE_OUTPUT_DIR = Path(__file__).resolve().parent / "geolife_results"
DEFAULT_SF_OUTPUT_DIR = Path(__file__).resolve().parent / "sf_results"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", choices=["geolife", "sf"], default="geolife")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=DEFAULT_GEOLIFE_DATA_DIR,
        help="Input dataset path: Geolife root directory or the SF Cabspotting CSV file.",
    )
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_GEOLIFE_OUTPUT_DIR)
    ...
    parser.add_argument(
        "--beijing-only",
        action="store_true",
        help="Only include users whose raw GPS points mostly lie inside the Beijing admin boundary (downloaded GeoJSON).",
    )
    parser.add_argument(
        "--beijing-min-fraction",
        type=float,
        default=0.7,
        help="With --beijing-only: keep a user if at least this fraction of points fall inside the boundary polygon.",
    )
    parser.add_argument(
        "--beijing-boundary-cache-dir",
        type=Path,
        default=None,
        help="Directory to cache downloaded Beijing GeoJSON (default: package data/ next to geolife_preprocess).",
    )
    parser.add_argument(
        "--beijing-boundary-url",
        type=str,
        default=None,
        help="Override URL for Beijing boundary GeoJSON (default: Aliyun DataV adcode 110000).",
    )
    parser.add_argument(
        "--projection",
...
def resolve_output_dir(base_dir: Path, run_name: str | None) -> Path:
    if run_name:
        ...
        return run_dir
    run_dir = base_dir / datetime.now().strftime("geolife_%Y%m%d_%H%M%S")
    run_dir.mkdir(parents=True, exist_ok=True)
    return run_dir
...
def save_user_label_exports(
    samples_df: pd.DataFrame,
    data_dir: Path,
    output_dir: Path,
    selected_user_ids: list[str] | None,
    dataset_name: str,
) -> None:
    """
    Persist transportation-mode labels per user:
    - Resampled points: counts (long + wide) by interval/interpolation/mode.
    - Overall fraction per user across modes.
    - Original Geolife ``labels.txt`` intervals (start/end/mode) for all included users.
    """
    ...
    root = Path(data_dir)
    users = list(selected_user_ids) if selected_user_ids else discover_labeled_users(root)
    rows: list[dict[str, object]] = []
    for uid in users:
        lab_path = root / uid / "labels.txt"
        if not lab_path.is_file():
            continue
        lab = parse_geolife_labels(lab_path)
        for _, r in lab.iterrows():
            st, en = r["start_time"], r["end_time"]
            rows.append(
                {
                    "user_id": uid,
                    "start_time": st.isoformat() if pd.notna(st) else "",
                    "end_time": en.isoformat() if pd.notna(en) else "",
                    "mode": str(r["mode"]),
                }
            )
    if rows:
        pd.DataFrame(rows).to_csv(output_dir / "user_original_label_intervals.csv", index=False)
...
def _init_scenario_training_losses(
    ...
    lines = [
        "# Geolife VQ-VAE training losses (per epoch). One block per (interval, interpolation, K).",
...
def main() -> None:
    args = parse_args()
    if args.dataset == "sf" and args.data_dir == DEFAULT_GEOLIFE_DATA_DIR and DEFAULT_SF_DATA_PATH.is_file():
        args.data_dir = DEFAULT_SF_DATA_PATH
    if args.dataset == "sf" and args.output_dir == DEFAULT_GEOLIFE_OUTPUT_DIR:
        args.output_dir = DEFAULT_SF_OUTPUT_DIR
    ...
    if torch is None:
        args.skip_vq = True
    if args.dataset == "sf" and args.beijing_only:
        raise SystemExit("--beijing-only is only supported for the Geolife dataset.")
    output_dir = resolve_output_dir(args.output_dir, args.run_name)
    dataset_path = output_dir / ("sf_samples.pkl" if args.dataset == "sf" else "geolife_samples.pkl")
    run_metadata_path = output_dir / "run_metadata.json"

    print(
        f"[experiment] dataset={args.dataset} output_dir={output_dir}",
        flush=True,
    )
    print(
    ...
    if args.dataset == "geolife" and args.beijing_only:
        selected_user_ids = list_beijing_user_ids(
            args.data_dir,
            min_fraction=args.beijing_min_fraction,
            boundary_cache_dir=args.beijing_boundary_cache_dir,
            boundary_url=args.beijing_boundary_url,
        )
        if not selected_user_ids:
            raise SystemExit(
                "No users passed the Beijing filter. Check --data-dir, or lower --beijing-min-fraction."
            )
        (output_dir / "beijing_user_ids.txt").write_text(
    ...
    save_user_label_exports(samples_df, Path(args.data_dir), output_dir, selected_user_ids, args.dataset)

    metadata = compute_dataset_metadata(samples_df)
    dataset_label = "Cabspotting / San Francisco" if args.dataset == "sf" else "Geolife"
    metadata.update(
        {
            ...
            "beijing_only": args.beijing_only,
            "beijing_min_fraction": args.beijing_min_fraction,
            "beijing_boundary_url": (
                args.beijing_boundary_url or DEFAULT_BEIJING_GEOJSON_URL if args.dataset == "geolife" else None
            ),
            "beijing_boundary_cache_dir": (
                str(
                    args.beijing_boundary_cache_dir
                    if args.beijing_boundary_cache_dir is not None
                    else Path(__file__).resolve().parent / "data"
                )
                if args.dataset == "geolife"
                else None
            ),
            "beijing_user_ids": selected_user_ids,
            "beijing_user_count": len(selected_user_ids) if selected_user_ids else None,
