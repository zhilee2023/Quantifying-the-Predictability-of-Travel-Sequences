...
from beijing_boundary import (
    DEFAULT_BEIJING_GEOJSON_URL,
    load_beijing_geometry,
    points_in_beijing_polygon,
)
...
def discover_labeled_users(data_dir: str | Path) -> list[str]:
    root = Path(data_dir)
    return sorted(path.parent.name for path in root.glob("*/labels.txt"))


def beijing_user_point_stats(
    data_dir: str | Path,
    user_id: str,
    *,
    geometry: object,
) -> tuple[int, int]:
    """Returns (num_points_inside_beijing_polygon, num_points_total) for all .plt files of this user."""
    ...
    for plt_path in sorted(traj_dir.glob("*.plt")):
        df = parse_geolife_trajectory(plt_path)
        if df.empty:
            continue
        n = len(df)
        total += n
        mask = points_in_beijing_polygon(
            df["latitude"].values,
            df["longitude"].values,
            geometry,
        )
...
def list_beijing_user_ids(
    data_dir: str | Path,
    *,
    min_fraction: float = 0.7,
    boundary_cache_dir: str | Path | None = None,
    boundary_url: str | None = None,
) -> list[str]:
    """
    Keep labeled users whose raw GPS points fall mostly inside the Beijing administrative boundary.

    Downloads and caches GeoJSON (default: Aliyun DataV adcode 110000) under ``boundary_cache_dir``.
    A user is retained if (points_inside / points_total) >= min_fraction and total > 0.
    """
    root = Path(data_dir)
    cache_dir = Path(boundary_cache_dir) if boundary_cache_dir is not None else Path(__file__).resolve().parent / "data"
    url = boundary_url if boundary_url is not None else DEFAULT_BEIJING_GEOJSON_URL
    geometry = load_beijing_geometry(cache_dir, url=url)
    candidates = discover_labeled_users(root)
    selected: list[str] = []
    for user_id in candidates:
        inside, total = beijing_user_point_stats(root, user_id, geometry=geometry)
        if total == 0:
            continue
        if inside / float(total) >= float(min_fraction):
            selected.append(user_id)
    return sorted(selected)


def parse_geolife_trajectory(path: str | Path) -> pd.DataFrame:
    columns = [
        "latitude",
        "longitude",
        "unused",
    ...
    df = pd.read_csv(path, skiprows=6, header=None, names=columns)
    if df.empty:
        return pd.DataFrame(columns=["timestamp", "latitude", "longitude", "altitude_ft"])
    ...
    df = df[["timestamp", "latitude", "longitude", "altitude_ft"]].reset_index(drop=True)
    return df


def parse_geolife_labels(path: str | Path) -> pd.DataFrame:
    labels = pd.read_csv(path, sep=r"\s+", engine="python")
    if labels.empty:
        return pd.DataFrame(columns=["start_time", "end_time", "mode"])

    labels["start_time"] = pd.to_datetime(
        labels["Start"].astype(str) + " " + labels["Time"].astype(str),
        format="%Y/%m/%d %H:%M:%S",
        errors="coerce",
    )
    labels["end_time"] = pd.to_datetime(
        labels["End"].astype(str) + " " + labels["Time.1"].astype(str),
        format="%Y/%m/%d %H:%M:%S",
        errors="coerce",
    )
    labels["mode"] = labels["Transportation"].astype(str).str.lower().str.strip()
    labels = labels.dropna(subset=["start_time", "end_time"]).copy()
    labels = labels.loc[labels["end_time"] >= labels["start_time"], ["start_time", "end_time", "mode"]]
    labels = labels.sort_values(["start_time", "end_time", "mode"]).reset_index(drop=True)
    return labels
...
def build_resampled_dataset(
    ...
    records = []
    user_ids = discover_labeled_users(data_root)
    ...
    for user_id in user_ids:
        ...
        labels_df = parse_geolife_labels(labels_path)
        for plt_path in sorted((user_dir / "Trajectory").glob("*.plt")):
            trajectory_df = parse_geolife_trajectory(plt_path)
            trajectory_id = f"{user_id}_{plt_path.stem}"
            for sample_interval in sample_intervals:
                for interpolation_method in interpolation_methods:
                    ...
                    if not segments.empty:
                        segments["source_file"] = str(plt_path)
