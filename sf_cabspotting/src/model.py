...
def model_train(
    ...
    Args:
        ...
        D_target (float): Threshold for recon_loss (mean squared Euclidean in **scaled** coords; same units as
            ``F.mse_loss * C`` aggregate). Pass (grid_cell_km / coordinate_scale_km)**2 for the grid bound.
    ...
    for epoch in range(num_epochs+pretrain_epochs):
        ...
        if coordinate_scale_km is not None:
            ...
            # With D_target = (grid_cell_km/scale)^2, target_RMSE_km equals grid_cell_km (tolerance in km)
            recon_log = (
