...
def generate_stable_vector_ar_matrices(R, D, max_radius=0.3, max_iter=1000000, rng=None):
    """
    生成稳定的向量 AR(R) 系数矩阵列表，供平稳与非平稳 Gaussian 实验共用。
    """
    if rng is None:
    ...
    for _ in range(max_iter):
        A_matrices = [uniform(-max_radius / R, max_radius / R, (D, D)) for _ in range(R)]

        companion = np.zeros((R * D, R * D), dtype=np.complex128)
        companion[:D, :] = np.hstack(A_matrices)
        for i in range(1, R):
...
def generate_piecewise_gaussian_markov_sequence(
    N,
    D,
    R,
    noise_std_pre=1.0,
    ...
    max_radius_pre=0.3,
    max_radius_post=0.55,
    A_matrices_pre=None,
    A_matrices_post=None,
    mean_shift_vector=None,
    rng=None,
):
    """
    生成分段非平稳 Gaussian vector-AR 序列。

    scenario:
      - matrix_shift: 切换点后更换 AR 矩阵
      - noise_shift: 切换点后增大噪声方差
      - mean_shift: 切换点后增加均值偏移
    """
    if rng is None:
    ...
    if A_matrices_pre is None:
        A_matrices_pre = generate_stable_vector_ar_matrices(R, D, max_radius=max_radius_pre, rng=rng)
    if A_matrices_post is None:
        if scenario == "matrix_shift":
            A_matrices_post = generate_stable_vector_ar_matrices(R, D, max_radius=max_radius_post, rng=rng)
        else:
            A_matrices_post = [mat.copy() for mat in A_matrices_pre]

    mean_shift = np.zeros(D, dtype=float)
    if scenario == "mean_shift":
        if mean_shift_vector is not None:
            mean_shift = np.asarray(mean_shift_vector, dtype=float)
        else:
            mean_shift = normal(0.0, mean_shift_scale, D)

    switch_index = int(np.clip(round(N * switch_frac), R + 1, N - 1))
    X = np.zeros((N, D))

    for t in range(R):
        X[t] = normal(0.0, noise_std_pre, D)

    for t in range(R, N):
        if t < switch_index:
            mats = A_matrices_pre
            curr_noise_std = noise_std_pre
            bias = 0.0
        else:
            mats = A_matrices_post
            curr_noise_std = noise_std_post if scenario == "noise_shift" else noise_std_pre
            bias = mean_shift

        for i in range(R):
            X[t] += mats[i] @ X[t - i - 1]
        X[t] += normal(0.0, curr_noise_std, D) + bias

    return X, A_matrices_pre, A_matrices_post, mean_shift
...
def generate_gaussian_markov_sequence(N, D, R, noise_std=1.0):
    ...
    """
    # 1. 生成稳定的AR系数矩阵
    A_matrices = generate_stable_vector_ar_matrices(R, D)
